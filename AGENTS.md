...
- GASフロントの公開だけで受信口は更新されません。`docs/google-apps-script/Code.gs` に変更がある場合はApps Scriptの既存デプロイを新バージョンへ更新し、URLと既存の秘密設定を保持します。WordPressだけ／GASだけの公開になった場合は未完了側を明記してください。
