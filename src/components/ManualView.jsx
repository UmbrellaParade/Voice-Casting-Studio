...
const OWNER_SECTIONS = [
  ...
  },
  {
    ...
    steps: [
      ...
      "初回だけ「フォーム自動作成」でOpenAI APIキーを保存し、「PC保存先を選択」からオーディションフォームサムネ一覧フォルダーを選びます。APIキーは暗号化され、入力後に再表示されません。",
      "役のキャラクター画像を登録してから「フォームを自動作成」を押すと、見本フォームの複製、役名反映、Googleフォーム用・SNS用画像の生成、Drive保存まで行います。処理には数分かかる場合があります。",
      "作成後は「フォームを編集」から内容を確認します。Google公式APIではテーマ画像を差し替えられないため、最後にフォームのテーマ画面で作成済みヘッダー画像を選びます。",
