...
const OWNER_SECTIONS = [
  ...
  },
  {
    ...
    steps: [
      ...
      "ElevenLabsを使う場合は「ElevenLabs Sound Effects連携」を開き、自分のAPIキーを登録します。WordPress版はオーナー別、GAS版は導入したオーナーのApps Scriptに保存し、声優さんの画面にはキーも設定欄も生成ボタンも渡しません。",
  ...
  },
  {
    ...
    steps: [
      ...
      "初回だけ「フォーム自動作成」で自分のOpenAI APIキーを保存します。生成画像はGoogle Driveの専用フォルダーへ自動保存されるため、PC保存先の選択は不要です。APIキーはサーバー側に保存し、入力後に再表示されません。",
...
function AccessSetupGuide() {
  return (
    <div className="manual-setup-guide">
      ...
      <section className="manual-setup-subsection">
        <div className="manual-setup-heading"><KeyRound size={18} /><div><h4>配布版は利用するオーナー自身のAPIキーを使用</h4><p>制作チームごとに自分のApps ScriptとDrive保存先を用意します。配布者のAPIキーは付属しません。画像生成やSE生成の料金は、設定したキーのアカウントで発生します。声優さん用の共有URLにキーを含めないでください。</p></div></div>
        <p>GAS版の初期設定は「設定」のGAS制作オーナー接続から行い、フォームの見本・保存先とOpenAIキーは「タスク」、ElevenLabsキーは「素材」で登録します。Code.gsだけでなく付属のappsscript.jsonも反映し、Apps ScriptでauthorizeVoiceCastStudioを実行してGoogleの権限を承認してください。GASのAPIキーはスクリプトプロパティに保存されるため、Apps Scriptの編集者を追加するとその人もキーにアクセスできます。</p>
        <div className="manual-setup-heading"><FileText size={18} /><div><h4>オーナー限定の接続情報ドキュメント</h4><p>Googleドキュメントを1つ作り、リンク共有をオフにしてオーナーだけが見られる状態にします。CodexへURLを渡せば、接続済みGoogle Driveまたはログイン済みブラウザから必要な項目を確認できます。APIキーそのものは設定欄へ直接入力する方法を優先してください。</p></div></div>
